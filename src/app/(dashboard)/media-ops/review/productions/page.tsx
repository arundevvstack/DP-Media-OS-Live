...
import { Film, ArrowRight, Loader2, Play } from "lucide-react";
...
export default function ReviewProductionsPage() {
    ...
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-black tracking-tight flex items-center gap-2">
          ...
          Review Center Productions
        </h1>
        <p className="text-muted-foreground mt-1">Select a production to launch its review workspace.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {productions.map((prod) => (
          <Card key={prod.id} className="group hover:border-primary transition-colors cursor-pointer" onClick={() => router.push(`/media-ops/review/${prod.id}`)}>
            <CardHeader>
