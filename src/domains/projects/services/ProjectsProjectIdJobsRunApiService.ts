...
import { productionAssetVersionRepository } from "@/domains/production/repositories/ProductionAssetVersionRepository";

export class ProjectsProjectIdJobsRunApiService {
...
async function jobRunHandler(
  ...
  try {
    ...
    const finalAssetVersion = await productionAssetVersionRepository.findFirst({
      where: { job_id: job.id },
