...
import { projectMemberRepository } from "@/domains/platform/repositories/ProjectMemberRepository";
import { userRepository } from "@/domains/identity/repositories/UserRepository";

export class ProjectsProjectIdStagesTransitionApiService {
...
async function stageTransitionHandler(
  ...
  try {
    ...
    const projectMember = await projectMemberRepository.findUnique({
      where: {
        project_id_user_id: {
