...
export class CrmClientCreateApiService {
    static async handlePOST(req: NextRequest) {
        try {
            ...
            if (!isAuthorized) {
                return { payload: { error: 'Forbidden: Insufficient permissions to onboard clients.' }, status: 403 };
            }
            ...
            return { payload: { data: result }, status: 201 };
        } catch (error: any) {
